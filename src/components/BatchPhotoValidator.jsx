...
const BatchPhotoValidator = () => {
  ...
  const fetchPhotos = useCallback(async (page = 1, options = {}) => {
    setLoading(true);
    try {
      ...
      // Si des filtres sont appliqués, les ajouter aux paramètres
      ...
      const response = await fetch(`${API_BASE_URL}/api/photos/list?${params.toString()}`, {
        headers: {
  ...
  }, [filterResolution, filterType, searchQuery, sortField, sortDirection]);
  ...
  useEffect(() => {
    // Revenir à la première page lorsque les filtres changent
    fetchPhotos(1);
  }, [filterResolution, filterType, searchQuery, sortField, sortDirection, fetchPhotos]);
          ...
          return (
            ...
            >
              <div className="aspect-w-1 aspect-h-1">
                <img
                  src={`${API_BASE_URL}/api/photos/${encodeURIComponent(photo.id)}?size=thumbnail`}
                  alt={photo.originalName}
                  className="object-cover w-full h-full"
                  loading="lazy"
                  onError={(e) => {
                    // Image load error
                    e.target.src = 'data:image/svg+xml;base64,';
                  }}
                />
              </div>
            ...
            {filtered.map(photo => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 py-2">
                    <a 
                      href={`${API_BASE_URL}/api/photos/${encodeURIComponent(photo.id)}`} 
                      target="_blank" 
  ...
  const renderBatchResults = () => {
    ...
    return (
      <div className="mt-4 p-4 bg-gray-100 rounded-lg">
        <h3 className="font-bold mb-2">Batch processing results</h3>
        
        <div className="flex justify-between mb-2">
          <div>
            <span className="font-medium">Successful:</span> {batchResults.success?.length || 0}
          </div>
          <div>
            <span className="font-medium">Failed:</span> {batchResults.failed?.length || 0}
          </div>
        </div>
    ...
    <div className="bg-white shadow-xl rounded-lg overflow-hidden w-full max-w-7xl">
      <div className="p-6">
        ...
        <div className="mb-4 flex items-center gap-2">
          ...
          >
            {Object.keys(selectedPhotos).length === photos.length ? 'Deselect all' : 'Select all'}
          </button>
          
          <span className="text-sm">
	   {Object.keys(selectedPhotos).length} photo(s) selected
          </span>
        </div>
        ...
        {loading ? (
          <div className="flex justify-center items-center py-10">
            ...
            <span className="ml-2 text-lg">Loading photos...</span>
          </div>
        ) : photos.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-10">
            <AlertTriangle className="w-12 h-12 text-yellow-500 mb-4" />
             <p className="text-lg">No photos to validate</p>
          </div>
        ) : (
          <>
            ...
            {!loading && photos.length > 0 && (
              <div className="mt-6 flex flex-col sm:flex-row justify-between items-center space-y-2 sm:space-y-0">
                <div className="text-sm text-gray-600">
                  Affichage de {(currentPage - 1) * 50 + 1} à {Math.min(currentPage * 50, totalPhotos)} sur {totalPhotos} photos
                </div>
